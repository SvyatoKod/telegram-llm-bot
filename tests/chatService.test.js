...
describe('ChatService.summarize', () => {
  test('пустая история → пустое резюме без вызова LLM', async () => {
    const llm = makeFakeLLM(async () => 'should not be called');
    const svc = new ChatService({ llmClient: llm });
    const summary = await svc.summarize({ history: [] });
    expect(summary).toBe('');
    expect(llm.chat).not.toHaveBeenCalled();
  });

  test('делает запрос с инструкцией о резюме', async () => {
    const llm = makeFakeLLM(async () => '  Краткое резюме.  ');
    const svc = new ChatService({ llmClient: llm, defaultSystemPrompt: 'sys' });
    ...
    expect(callArgs[0]).toEqual({ role: 'system', content: 'sys' });
    expect(callArgs.find((m) => /резюме/i.test(m.content))).toBeDefined();
    expect(callArgs[callArgs.length - 1]).toEqual({
      role: 'user',
      content: 'Кратко резюмируй диалог выше.',
    });
  });

  test('ошибка LLM при summarize прокидывается', async () => {
    const llm = makeFakeLLM(async () => {
      throw new Error('summary failed');
    });
    const svc = new ChatService({ llmClient: llm });
    await expect(
      svc.summarize({ history: [{ role: 'user', content: 'q' }] }),
    ).rejects.toThrow('summary failed');
  });

  test('non-string ответ от LLM возвращается как пустая строка', async () => {
    const llm = makeFakeLLM(async () => 42);
    const svc = new ChatService({ llmClient: llm });
    const summary = await svc.summarize({ history: [{ role: 'user', content: 'q' }] });
    expect(summary).toBe('');
  });
});
